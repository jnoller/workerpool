...
class TestMisc(unittest.TestCase):
    ...
    def test_harvest(self):
        ...
        for _ in range(100):
            q.put(1)
        self.assertEqual(sum([i for i in workerpool.iterqueue(q)]), 100)
...
class TestWorkerThreadPool(unittest.TestCase):
    ...
    def test_basic_work(self):
        ...
        """
        x = workerpool.WorkerPool(self.wc)
        for i in range(self.wc):
            x.inbox.put((_tfunc, [1], {}))
        x.exile()
        results = [i for i in workerpool.iterqueue(x.outbox)]
    ...
    def test_prodcom(self):
        ...
        """
        prod = workerpool.WorkerPool(self.wc)
    ...
    def test_circle(self):
        ...
        inbox = Queue.Queue()
        outbox = Queue.Queue()
        prod = workerpool.WorkerPool(self.wc, inbox=outbox, outbox=inbox)
        com = workerpool.WorkerPool(self.wc, inbox=inbox, outbox=outbox)
        for i in range(10):
...
class TestSummoningPool(unittest.TestCase):
    ...
    def test_sp_basic(self):
        x = workerpool.SummoningPool(1, 10, rate=2, ratio=2, interval=0)
        x.pause()
        for _ in range(100):
            x.inbox.put((_lazyfunc, [], {}))
        x.run()
